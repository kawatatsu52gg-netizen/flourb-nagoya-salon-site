...
import { pickServiceIdForDiagnosis, type DiagnosisServiceHint } from "@/lib/booking";
...
const DIAGNOSIS_HINTS = new Set<DiagnosisServiceHint>(["herb", "kogao", "both"]);

export function BookingClient() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const autoOpenedSlotsRef = useRef(false);
  ...
  useEffect(() => {
    async function loadServices() {
      ...
      try {
        ...
        const rawHint = searchParams.get("diagnosis");
        const hint =
          rawHint && DIAGNOSIS_HINTS.has(rawHint as DiagnosisServiceHint) ? (rawHint as DiagnosisServiceHint) : undefined;
        const picked = hint ? pickServiceIdForDiagnosis(safeServices, hint) : undefined;
        setServiceId(picked ?? safeServices[0].id);
      } catch {
        setServices(FALLBACK_SERVICES);
        const rawHint = searchParams.get("diagnosis");
        const hint =
          rawHint && DIAGNOSIS_HINTS.has(rawHint as DiagnosisServiceHint) ? (rawHint as DiagnosisServiceHint) : undefined;
        const picked = hint ? pickServiceIdForDiagnosis(FALLBACK_SERVICES, hint) : undefined;
        setServiceId(picked ?? FALLBACK_SERVICES[0].id);
      } finally {
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    if (!openSlots || autoOpenedSlotsRef.current) return;
    autoOpenedSlotsRef.current = true;
    setModalOpen(true);
  }, [loading, serviceId, searchParams]);
